...
public class NewMoviesPresenter extends RxPresenter<MoviesContract.View> implements MoviesContract.Presenter {

    private final RetrofitHelper helper;

    @Inject
    public NewMoviesPresenter(RetrofitHelper helper) {
        this.helper = helper;
    }

    @Override
    public void getMoviesList(int start,int count, final boolean isLoadMore) {
        Subscription subscribe = helper.getNewMovies(start,count)
